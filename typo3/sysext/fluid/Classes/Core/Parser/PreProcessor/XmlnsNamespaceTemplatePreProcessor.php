...
class XmlnsNamespaceTemplatePreProcessor implements TemplateProcessorInterface
{
    ...
    public function preProcessSource($templateSource)
    {
        ...
        $matched = preg_match_all('/<([a-z0-9]+)\\s*' . $namespacePattern . '[^>]*>/i', $templateSource, $matches, PREG_SET_ORDER);
        if ($matched) {
            $namespaces = array();
            preg_match_all('/' . $namespacePattern . '/i', $matches[0][0], $namespaces, PREG_SET_ORDER);
            ...
            if (strpos($matches[0][0], 'data-namespace-typo3-fluid="true"') || !strpos($matches[0][0], 'data-namespace-typo3-fluid')) {
                ...
                foreach ($matches as $match) {
                    $templateSource = str_replace($match[0], '', $templateSource);
                }
            }
        }
