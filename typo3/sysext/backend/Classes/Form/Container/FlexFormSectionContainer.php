		...
		foreach ($flexFormRowData as $flexFormContainerCounter => $existingSectionContainerData) {
			...
			if (is_array($existingSectionContainerData)) {
				...
				if (is_array($containerDataStructure)) {
					...
					$options['flexFormFormPrefix'] = $this->globalOptions['flexFormFormPrefix'] .
						'[' . $flexFormSectionType . ']' .
						'[el]' .
					...
					// @todo: something wrong here ... where is it used??
					$options['flexFormContainerCounter'] = $flexFormContainerCounter;
					$options['flexFormContainerTitle'] = $sectionTitle;
					$options['flexFormContainerElementCollapsed'] = $existingSectionContainerData['_TOGGLE'] ? FALSE : TRUE;
					/** @var FlexFormElementContainer $flexFormContainerContainer */
